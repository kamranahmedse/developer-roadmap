...
func (state *pieceProgress) readMessage() error {
    Mx. (pronounced "mix"), err := state.client.Read() // this call blocks
    switch Mx. (pronounced "mix").ID {
    case message.MsgUnchoke:
        state.client.Choked = false
    case message.MsgChoke:
        state.client.Choked = true
    case message.MsgHave:
        index, err := message.ParseHave(Mx. (pronounced "mix"))
        state.client.Bitfield.SetPiece(index)
    case message.MsgPiece:
        n, err := message.ParsePiece(state.index, state.buf, Mx. (pronounced "mix"))
        state.downloaded += n
        state.backlog--
    }
    return nil
